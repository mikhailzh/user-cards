...
typedef IndexedWidgetWithAnimationBuilder = Widget Function(
    BuildContext context, int index, Stream<double>? scrollProgress, PageType type);
...
class TikTokStyleFullPageScroller extends StatefulWidget {
  ...
  @override
  _TikTokStyleFullPageScrollerState createState() => _TikTokStyleFullPageScrollerState();
}

class _TikTokStyleFullPageScrollerState extends State<TikTokStyleFullPageScroller> with SingleTickerProviderStateMixin {
  late Size _containerSize;
  late double __cardOffset = 0;

  double get _cardOffset => __cardOffset;

  set _cardOffset(double value) {
    _streamProgressController.add(value / _containerSize.height);
    __cardOffset = value;
  }

  late final _streamProgressController = StreamController<double>();
  late final _streamProgress = _streamProgressController.stream.asBroadcastStream();

  late double _dragStartPosition = 0;
        ...
        children: <Widget>[
          if (_cardIndex > 0)
            Positioned(
              ...
              child: SizedBox.fromSize(
                  size: _containerSize,
                  child: widget.builder(context, _cardIndex - 1, _streamProgress, PageType.upper)),
            ),
          Positioned(
            top: _cardOffset,
            child: GestureDetector(
              child: SizedBox.fromSize(
                  size: _containerSize, child: widget.builder(context, _cardIndex, _streamProgress, PageType.central)),
              ...
              onVerticalDragEnd: (DragEndDetails details) {
                bool positiveDragThresholdMet = (_cardOffset < -_containerSize.height * widget.swipePositionThreshold ||
                    details.primaryVelocity! < -widget.swipeVelocityThreshold);

                bool negativeDragThresholdMet = (_cardOffset > _containerSize.height / widget.swipePositionThreshold ||
                    details.primaryVelocity! > widget.swipeVelocityThreshold);
                ...
                if (positiveDragThresholdMet && _cardIndex < widget.contentSize - 1) {
                  ...
                  if (_cardIndex == 0) {
                    ...
                    _pendingEvent = ScrollEvent(ScrollDirection.BACKWARDS, ScrollSuccess.FAILED_END_OF_LIST, 0);
                  ...
                  _pendingEvent =
                      ScrollEvent(ScrollDirection.FORWARD, ScrollSuccess.FAILED_END_OF_LIST, widget.contentSize - 1);
                  _state = DragState.animatingToCancel;
                } else {
                  // Thresholds not met so relaxing back to initial state
                  _pendingEvent = ScrollEvent(_cardOffset < 0 ? ScrollDirection.FORWARD : ScrollDirection.BACKWARDS,
                      ScrollSuccess.FAILED_THRESHOLD_NOT_REACHED, null);
                  _state = DragState.animatingToCancel;
                }
                setState(() {
                  _dragState = _state;
          ...
          if (_cardIndex < widget.contentSize - 1)
            Positioned(
              ...
              child: SizedBox.fromSize(
                size: _containerSize,
                child: widget.builder(context, _cardIndex + 1, null, PageType.lower),
              ),
            ),
        ],
      );
  ...
  void _createAnimation() {
    ...
    _animation = Tween<double>(begin: _cardOffset, end: _end).animate(_animationController)
